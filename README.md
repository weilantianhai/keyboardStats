...
| 模式 | 启动方式 | 职责 | 内存实测 |
|---|---|---|---|
| **记录进程** | `--record`（GUI 启动时自动拉起） | 全局钩子 + 计数 + 每 5 秒落盘 + **托盘图标** | **约 18 MB** |
...
注册的命令带 `--record`，**开机只启动无界面记录进程 + 托盘图标（约 18 MB），不弹窗口**。
...
build\KeyboardStats.exe --record        # 无界面记录进程（自启动用），只在托盘，约 18 MB
